...
generated by `sagctl login`, not an example file in the repo). The structure below only
has value as an illustration of the packaging IDEA, not the real directory tree:
...
1. ~~`scripts/sagctl.py`~~ → `scripts/sagctl/` (package) + `scripts/sagw_server.py` (the
   ...
   REVIEW-OPUS).
2. 5 skills + `.mcp.json` + plugin manifest + marketplace.json + PATH-install shim —
   correct.
